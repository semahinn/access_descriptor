...
class User implements UserInterface {
  ...
  public function __construct(int $id, string $uuid, string $display_name, string $username, array $roles) {
    $this->id = $id;
    $this->uuid = $uuid;
  ...
  public static function john() {
    $user = static::USERS[1];
    return new User($user['id'], $user['uuid'], $user['display_name'], $user['username'], $user['roles']);
  }
